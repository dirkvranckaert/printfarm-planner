...
| `link-transition.js` | Single source of truth for printer-link lifecycle: `LINK_ACTIVE_STATUSES`, `eligibleToStart`, `eligibleToPause`, `applyRunningTransition`, `applyPauseTransition`. Imported by `server.js`, `db.js` AND the tests |
...
- **Printer-link lifecycle — one invariant, one module.** **Invariant: a job in an inactive status must NEVER hold a live `linked_printer_id`.** Violate it and the next print re-grabs the old job (seen in prod: `Post Printing` job kept its link, new print flipped it back to `Printing`).
  - `link-transition.js` is the ONLY definition of link semantics. `LINK_ACTIVE_STATUSES` = `Awaiting Printer`, `Printing`, `Paused`. `server.js` (SSE stage handler), `db.js` (backfill) and `tests/awaiting-printer.test.js` all import it — **never re-implement or mirror it**. A hand-copied `simulateRunningTransition` test mirror once let tests stay green while prod diverged; it was deleted, don't reintroduce that shape.
  - RUNNING edge: `eligibleToStart` = pending `Awaiting Printer` (still in window) or resuming `Paused` ONLY. PAUSE edge: `eligibleToPause` = `Printing` ONLY — pausing a stale job would let PAUSE→RUNNING re-admit it to `Printing`.
  - Unlink on status leave lives in PUT **and** PATCH `/api/jobs/:id`, folded INTO the primary `UPDATE` statement — one atomic statement, never a follow-up `UPDATE` (that leaves a crash window where a stale link survives, and the one-time backfill can no longer heal it).
  - `clearLink = !LINK_ACTIVE_STATUSES.has(status)` is deliberately NOT gated on `status !== undefined`: a PUT omitting `status` binds SQL NULL → must also unlink.
- **`PUT /api/jobs/:id` is a full-row replace, not a partial update.** Every omitted field is nulled/defaulted. Both web callers always send `status`. Partial edits go through `PATCH`. Any new caller must send the whole row or use `PATCH`.
...
- **Regression tests must be revert-confirmed.** After writing a guard + its test, revert the guard and re-run: the test MUST fail. Structurally-unfailable tests (asserting on a hand-written mirror of prod logic, or on a condition the fixture can't violate) passed review rounds here while proving nothing. Test the production module directly — import it, don't copy it.
...
- **`LINK_ACTIVE_STATUSES` ↔ backfill marker version are COUPLED (`migration.stale_link_backfill_v1` in `db.js`):** the one-time backfill NULLs `linked_printer_id` for rows outside `LINK_ACTIVE_STATUSES`, marker-guarded via the `settings` table, runs at `require('./db')`. Marker is **permanent per-DB and prod already consumed `_v1`** — so ANY future change to `LINK_ACTIVE_STATUSES` (add or remove a status) REQUIRES bumping the key to `_v2`, else rows that only become stale under the NEW set never heal. A test in `tests/awaiting-printer.test.js` pins the set and fails loudly to remind you.
- **`GET /api/jobs` returns the FULL unbounded job history on every render (known debt):** every panel (today/week/month/upcoming/queue) consumes the same payload, so network + `JSON.parse` cost grows linearly with history forever. Client-side conflict detection is already O(n log n); this payload is the remaining scaling wall. Fix = server-side date/status scoping — deliberately not done yet; don't "optimise" one panel by forking the fetch.
- **`detectConflicts` (`public/app.js`) is a per-printer sweep line, keep it that way:** group by printer, ONE `Date` parse per job, sort by buffer-inclusive start, running `maxEnd` + `witness` id. It re-runs on every `renderDay()` and every SSE stage frame, so an all-pairs scan hangs the tab as history grows (was a prod "Page Unresponsive"). Two traps if you touch it: non-finite intervals MUST be dropped BEFORE the sort (a NaN poisons the comparator), and finite-but-reversed intervals (`end < start`) route to an exact both-inequality fallback so behaviour matches the old all-pairs scan.
