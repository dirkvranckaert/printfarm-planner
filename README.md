...
2. **Every 60s while paused** (`pause.pauseTick`, driven by a dedicated 60-second `setInterval` in `server.js`, kept separate from the upcoming-notifications interval because the latter short-circuits when the user disables that push category): for every paused job, the server rewrites `end = now + paused_remaining_ms` and shifts `start` to preserve the original duration. The day-view bar drifts forward with the now-line. Downstream Planned/Awaiting jobs cascade backward via `scheduling.pushBackChain`, same as the RUNNING realign.
